#include "../include/test_optimization_algorithm.h"
...
int test_optimization_algorithm(struct Product *data, int num_of_rows, float total_budget){
    ...
    // Data with which optimization function output needs to be compared for validating the result
    int temp_data[]={29,29,15};
    // Flag value denoting the success or failure of the test
    int flag=0;
    ...
    for (i=0; i<num_of_rows; i++){  /* For loop iterates through each item of the
        ...
        if(data[i].restock_amount!=temp_data[i]){
            flag=-1;
        }
        puts("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
    }
